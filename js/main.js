$(document).ready(function() {
    //main-visual 숫자 카운트
    setCount('.app-viewinfo ul li:eq(0) .app-num', 1000, 129);
    setCount('.app-viewinfo ul li:eq(1) .app-num', 1000, 129);
    ...
    function setCount(selector, duration, step) {
        ...
        timerId = setInterval(function() {
            if (numNow > numTarget) {
                ...
                numNow += numStep; //소수를 넣어야 뒷자리가 자주 바뀜       
            }
        }, speed);
    }
    ...
    function setImageSlide(selector, first, status, speed) {
        ...
        $(selector).find('a.slide-btn').on('click', function() {
        ...
        function showSlide(n) {
            ...
            $('.app-info1 .app-slider-bg .app-mobile-slider img').css({'opacity': 0.5}).stop().attr('src', 'img/slider' + n + '.png').animate({'opacity': 1});
            slideNow = n;
